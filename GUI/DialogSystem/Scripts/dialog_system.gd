...
func set_dialog_text(d: DialogText):
	content.text = d.text
...
func set_dialog_choice(d: DialogChoice):
	...
	for i in d.dialog_branches.size():
		...
		new_choice.pressed.connect(on_dialog_choice_selected.bind(d.dialog_branches[i]))
		choice_options.add_child(new_choice)
		
	await get_tree().process_frame
	
	choice_options.get_child(-1).grab_focus()
...
func on_dialog_choice_selected(d: DialogBranch):
	choice_options.visible = false
